...
Task ID: 8
Agent: full-stack-developer
Task: Update all pages to import from single content.ts file

Work Log:
- Read worklog.md and existing content.ts + all 5 page files + footer/navbar/search-modal to plan the migration
- home.tsx: Switched import from `@/lib/data/school` to `@/lib/content`; added HERO, WHY_CHOOSE imports; added DynamicIcon import; removed unused Lucide imports (Calendar, CheckCircle2, BookOpen, Users, Trophy, HeartHandshake); hero text now uses HERO.eyebrow/description/primaryButton/secondaryButton/propheticQuote/propheticSource; foundation card labels now come from HERO.foundationLabels (mapped to the 4 colored chips); the 4 pillars section now maps over WHY_CHOOSE and renders icons via <DynamicIcon name={v.icon} />
- about.tsx: Switched import to content.ts; added STATS + MISSION imports; deleted the inline QUICK_STATS array and inlined `STATS.map(...)` in the JSX instead; mission card paragraph now uses MISSION.description. CORE_VALUES / FACILITIES already used DynamicIcon (string icons), so no rendering changes were needed there.
- academics.tsx: Switched import to content.ts; added GRADE_LEVELS, SCIENCE_FEATURES, LANGUAGES, LABS, ARTS_FEATURES imports; removed the 5 large inline arrays + their Lucide component icon imports (Microscope, Leaf, Sprout, Sun, Languages as LanguagesIcon, Music, Palette, Drama, Library, GraduationCap); SCIENCE_FEATURES / LANGUAGES / ARTS_FEATURES now render icons via <DynamicIcon name={...} /> instead of <feature.icon /> / <lang.icon /> / <feat.icon />. LABS still uses lab.iconName (string), already rendered through DynamicIcon, no change needed.
- news.tsx: Switched import to content.ts; added NEWS_CATEGORIES and ANNOUNCEMENTS imports; deleted the two inline arrays; updated the inline comment about the source file path; added a `navy` tone branch to the announcement card className logic because content.ts includes a tone='navy' entry that the original tone switch didn't handle (would have rendered unstyled).
- gallery.tsx: Switched import to content.ts; removed the unused SPAN_PATTERNS constant (masonry layout was already replaced by a clean grid, so the array was dead code).
- contact.tsx: Switched import to content.ts; added CONTACT_INFO, SOCIAL_LINKS, DEPARTMENTS_CONTACT, CONTACT_SUBJECTS imports; added DynamicIcon import; deleted the inline CONTACT_INFO, SOCIAL_LINKS, DEPARTMENTS arrays; removed now-unused Lucide imports (Phone, Mail, Clock, Facebook, Twitter, Instagram, Youtube, Building2, GraduationCap, Calculator, Bus); kept MapPin (map section), Send (submit button), ArrowRight (CTA), MessageSquare (form heading). Contact cards now render icon via DynamicIcon and use info.label / info.lines from content.ts; the action/href ("Get Directions", "Call Now", "Send Email") is preserved via a small CONTACT_ACTIONS helper keyed by label (since content.ts doesn't carry action/href, this keeps the visual action links intact). Social buttons render via <DynamicIcon name={s.icon} />. Subject dropdown now maps over CONTACT_SUBJECTS. Department table now uses DEPARTMENTS_CONTACT and DynamicIcon; the "Hours" column was removed because content.ts's DEPARTMENTS_CONTACT has no hours field.
- footer.tsx: Switched import to content.ts; added NEWSLETTER_TOPICS import; newsletter band now shows the topics joined by "·" as a small caption under the subscribe heading. (Navbar and search-modal already import only from nav-store / page-registry, not from data/school — verified with a project-wide grep — so no changes were needed there.)
- Deleted src/lib/data/school.ts and the now-empty src/lib/data directory.
- Ran `bun run lint` (clean, exit 0) and `bunx tsc --noEmit` (only errors in examples/ and skills/, src/ is clean). Verified dev.log shows ✓ Compiled entries with no errors and `curl http://localhost:3000/` returns HTTP 200 with "Guiding / Hearts / Minds / Hamza School" in the markup.

Stage Summary:
- All 5 pages (home, about, academics, news, gallery, contact) + footer now import their data from the single src/lib/content.ts file. The old src/lib/data/school.ts is deleted.
